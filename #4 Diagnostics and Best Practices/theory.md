...
Если контейнер `api` падает с ошибкой PHP или базы данных, вы увидите трассировку прямо в терминале. Флаг `-f` показывает новые строки в реальном времени — удобно держать во втором окне, пока воспроизводите проблему.

> Совет: `docker compose logs --since=5m api` поможет вытащить только свежие сообщения, если в сервисе много шума.
...
- `adduser -D` (в Alpine) или `useradd -m` (в Debian) создаёт пользователя без пароля и домашнюю директорию.
...
| **Локаль и часовой пояс** | Неправильное форматирование дат, кривые сравнения строк | `docker exec` → `locale`, `date` | Установите пакеты локалей (`apt-get install locales`), задайте `TZ=Europe/Moscow`, смонтируйте `/etc/localtime` при необходимости |
